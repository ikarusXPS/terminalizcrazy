...
const WelcomeMessage = `Willkommen bei TerminalizCrazy!
...
const CompletionMessage = `Tutorial abgeschlossen!
...
const WelcomeDescription = `TerminalizCrazy ist ein KI-gestuetztes Terminal.
...
const FirstQuestionDescription = `Stellen Sie Ihre erste Frage an die KI.
...
const ExecuteCommandDescription = `Die KI hat einen Befehl vorgeschlagen.
...
const RiskConfirmationDescription = `Bei riskanten Befehlen erscheint eine Bestaetigung.
...
const HistoryNavigationDescription = `Mit den Pfeiltasten navigieren Sie durch Ihre History.
...
const ClipboardDescription = `Mit Ctrl+Y kopieren Sie den letzten Befehl ins Clipboard.
...
const SessionSharingDescription = `Mit Ctrl+S starten Sie die Zusammenarbeit.
...
const SessionRestoreDescription = `Beim Start koennen Sie eine vorherige Session laden.
...
const CompleteDescription = `Sie sind bereit!
